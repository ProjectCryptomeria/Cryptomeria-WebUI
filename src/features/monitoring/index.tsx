...
import { BottomPanel } from '../../components/ui/BottomPanel'; // 追加
...
const MonitoringLayer: React.FC<MonitoringLayerProps> = ({ setDeployedNodeCount }) => {
    ...
    return (
        <div className="flex h-full w-full overflow-hidden relative text-slate-800">
            ...
            {/* Side Panel (Existing implementation) */}
            <div className={`flex-shrink-0 border-l border-slate-200 bg-white relative z-20 transition-all duration-300 overflow-hidden flex flex-col ${isSidePanelOpen ? 'w-96' : 'w-0'}`}>
                <div className="p-5 border-b border-slate-100 flex justify-between items-center shrink-0">
                    ...
                    <button onClick={() => setIsSidePanelOpen(false)} className="text-slate-400 hover:text-slate-600 p-2 rounded-lg hover:bg-slate-50 transition-colors">
                        <X className="w-5 h-5" />
                    </button>
                </div>
