...
const EconomyLayer: React.FC<EconomyLayerProps> = ({
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      {/* Private Key Modal */}
      <Modal
        isOpen={!!viewPrivateKeyUser}
        onClose={() => setViewPrivateKeyUser(null)}
      ...
      >
        ...
        <div className="p-6 pt-0">
          <div className="mb-6">
            ...
            <div className="font-mono text-xs bg-slate-100 p-3 rounded-xl break-all border border-slate-200 text-slate-600">
          ...
          <div className="bg-slate-900 rounded-2xl p-6 border border-slate-700 relative group shadow-inner">
            <div className="font-mono text-emerald-400 text-sm leading-relaxed break-words filter blur-[3px] group-hover:blur-0 transition-all duration-500 select-all">
              witch collapse practice feed shame open despair creek road again ice least monster
              budget hero
            </div>
          ...
          <div className="mt-8 flex justify-end">
            <Button variant="secondary" onClick={() => setViewPrivateKeyUser(null)}>
              閉じる
            </Button>
          </div>
        </div>
        ...
        <Card className="p-6 bg-gradient-to-br from-white to-blue-50/50 border-blue-100 lg:col-span-1 h-full">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3 text-slate-700">
              <div className="p-2 bg-white rounded-lg shadow-sm border border-blue-100 text-blue-500">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                ...
                onChange={() => setWatchdogEnabled(!watchdogEnabled)}
          ...
          <div className="text-xs text-slate-500 leading-relaxed font-medium mt-4">
            {watchdogEnabled ? (
              <span className="text-emerald-600 flex items-center gap-1">
                <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" /> 監視中:
                残高不足時に自動でFaucetを実行します。
              </span>
            ) : (
              <span className="text-slate-400">
                機能停止中: ガス欠により停止するリスクがあります。
              </span>
            )}
          </div>
        </Card>
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-bold text-slate-800 px-2 flex items-center gap-2">
          <Wallet className="w-5 h-5 text-slate-400" /> User Wallets{' '}
                ...
                <tr key={user.id} className={TableStyles.Row}>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div
                          className="text-xs text-slate-400 font-mono flex items-center gap-1 mt-0.5 group cursor-pointer"
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="font-mono font-bold text-slate-800 text-lg">
                      ...
                      <span className="text-xs text-slate-400 font-sans">TKN</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-right">
      ...
      <Card className="overflow-hidden border-slate-200 shadow-none bg-slate-50">
        <div
          className="px-6 py-4 flex justify-between items-center cursor-pointer hover:bg-slate-100 transition-colors select-none"
        ...
        >
          <h3 className="font-bold text-slate-600 flex items-center gap-2 text-sm uppercase tracking-wider">
            <Server className="w-4 h-4" /> System Accounts (Infra & Pool)
          </h3>
          {isSystemExpanded ? (
        ...
        {isSystemExpanded && (
          <div className="overflow-x-auto animate-in slide-in-from-top-2 duration-200 border-t border-slate-100 bg-white">
            <table className="w-full text-left text-sm">
              <thead className={TableStyles.Header}>
                <tr>
                  <th className="px-6 py-3 whitespace-nowrap">Name / Role</th>
                  <th className="px-6 py-3 whitespace-nowrap">Address</th>
                  <th className="px-6 py-3 text-right whitespace-nowrap">Balance</th>
                  ...
                  <tr key={acc.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4 font-medium text-slate-700 flex items-center gap-2 whitespace-nowrap">
                      {acc.type === 'faucet_source' ? (
                        <div className="p-1.5 bg-yellow-50 rounded text-yellow-500">
                    ...
                    <td className="px-6 py-4 text-right whitespace-nowrap">
                      {acc.type !== 'faucet_source' ? (
                        ...
                        <span className="text-xs text-slate-300 font-bold italic px-2">
                          Global Pool
                        </span>
                      )}
                    </td>
